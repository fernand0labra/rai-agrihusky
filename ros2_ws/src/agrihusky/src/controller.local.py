...
class HuskyController(Node):
    ...
    def __init__(self, rate=10, maxSpeed=0.5, maxAngVel=0.5, goalDistance=0.2, slowDownDistance=0.7, turnInPlaceAngle=0.5):
        ...
        self.positionSub = self.create_subscription(Odometry, '/husky/odom', self.odometryCallback, self.rate)           # Odometry Callback
        if not inSimulation:
            self.orientationSub = self.create_subscription(Imu, '/imu/data/repub', self.imuCallback, self.rate)          # IMU Callback
    ...
    def waypointRequest(self):
        ...
        self.waypointMsgRequest.waypoint.y = self.y + 0.1 * np.sign(self.y)
        self.waypointMsgRequest.waypoint.x = self.x
...
def main(args=None):
    try:
        ...
        huskyController = HuskyController(rate=25, maxSpeed=0.7, maxAngVel=0.6, goalDistance=0.05, slowDownDistance=0.25, turnInPlaceAngle=0.2)    
